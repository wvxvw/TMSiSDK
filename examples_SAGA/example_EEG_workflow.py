...
Example : This example shows the functionality of the impedance plotter and the 
            ...
            Measurement data is saved to poly5 or xdf-file, depending on user input.
...
try:
    ...
    window = plotters.ImpedancePlot(figurename = 'An Impedance Plot', device = dev, layout = 'head', file_storage = True)
    window.show()
    ...
    if file_format.lower()=='poly5':
        file_writer = FileWriter(FileFormat.poly5, "../measurements/example_EEG_workflow.poly5")
    elif file_format.lower()=='xdf':
        file_writer = FileWriter(FileFormat.xdf, "../measurements/example_EEG_workflow.xdf", add_ch_locs=True)
    else:
